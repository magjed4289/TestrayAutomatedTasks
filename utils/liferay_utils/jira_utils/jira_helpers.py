...
from utils.liferay_utils.jira_utils.jira_constants import CustomField, Status, Instance, Transition, Strings, IssueTypes
...
def create_task_for_flaky_test(epic,jira_component,summary,description,jira_conn):
    task = __initialize_task(
        epic, jira_component, summary, issuetype=IssueTypes.Task, description=description
    )
    created_issue = jira_conn.create_issue(fields=task)
    return created_issue

def __initialize_task(epic, component, summary, issuetype=IssueTypes.Task, description=''):
    task_in_current_testing_epic = {
        'project': {'key': 'LPD'},
        'summary': summary,
        'description': description,
        'issuetype': {'name': issuetype},
    ...
    return task_in_current_testing_epic
...
def initialize_subtask_patch_release(story, summary):
    ...
    subtask_test_validation = __initialize_subtask_technical_task(story, '', summary + ' - Validation', description)
